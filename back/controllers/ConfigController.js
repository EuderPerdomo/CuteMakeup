...
const crear_banner_admin = async function (req, res) {
  console.log('Crear Banner Admin', req.body)
  if (req.user) {
    let data = req.body;
    //data.slug = data.titulo.toLowerCase().replace(/ /g, '-').replace(/[^\w-]+/g, '');
...
const registro_imagen_banner_admin = async function (req, res) {
  console.log('registroimagen', req.body, req.params['id_banner'])
  if (req.user) {
    let data = req.body;
...
const actualizar_item_banner_admin = async function (req, res) {
  ...
  if (req.user) {
    ...
    console.log('actualizar', data, id_banner, id_item)
    try {
      ...
      if (req.files) {
        ...
        console.log('Item Obtenido', item.galeria[0].imagen)
